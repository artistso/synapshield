# SynapShield Authors & Contributors

## Lead Author

**Steven Owens**  
Computational Bioengineering Laboratory  
Ocean Shores, Washington, USA  
ORCID: [0009-0006-0211-4812](https://orcid.org/0009-0006-0211-4812)  
GitHub: [@artistso](https://github.com/artistso)  
Email: artistso@github.com  

**Roles (CRediT):** Conceptualization, Methodology, Software, Validation, Formal Analysis, Investigation, Data Curation, Writing – Original Draft, Writing – Review & Editing, Visualization, Project Administration

---

## Dedication

**Richard**  
Ocean Shores Medical Supply  
Ocean Shores, Washington, USA  

Patient advocate, inspiration, and namesake dedication.  
*"Richard, you've spent your life supplying medical equipment to help others heal. Now, we're caring for you."*

**Role:** Patient Partner, Community Advocacy, Inspiration

> This work is dedicated to Richard, whose courage in the face of Parkinson's inspired this project. May SynapShield intercept what Parkinson's destroys.

---

## Preprint Information

- **medRxiv submission:** June 30, 2026
- **Status:** Under review
- **DOI:** 10.1101/TBD (pending)
- **License:** MIT
- **Repository:** https://github.com/artistso/synapshield
- **Live Demo:** https://artistso.github.io/synapshield/

---

## Affiliations

1. Computational Bioengineering Laboratory, Ocean Shores, WA, USA
2. SynapShield Research Consortium (Open-source)

---

## Corresponding Author

Steven Owens  
Computational Bioengineering Laboratory  
Ocean Shores, Washington, USA  
ORCID: https://orcid.org/0009-0006-0211-4812  
Email: artistso@github.com

---

## Acknowledgements

- PPMI Database (Parkinson's Progression Markers Initiative)
- UK Biobank
- Bionics Institute (vagus nerve stimulation research)
- Ocean Shores community – 127+ supporters
- FEniCSx Project
- Every researcher fighting neurodegeneration

---

## Funding

Self-funded / open-source community. No commercial funding. No conflicts of interest declared.

---

## Data Availability

All code, simulations, and documentation are open-source under MIT License at:
https://github.com/artistso/synapshield

Simulation data: `simulations/results/`
Preprint: medRxiv (pending DOI)

---

## How to Cite

**APA (7th):**

Owens, S. (2026). *SynapShield: Intercepting Parkinson's Disease at the Gut-Brain Interface — A Bioengineered Hydrogel Approach with Computational Validation*. medRxiv. https://doi.org/10.1101/TBD

**MLA:**

Owens, Steven. "SynapShield: Intercepting Parkinson's Disease at the Gut-Brain Interface." *medRxiv*, 30 June 2026. https://github.com/artistso/synapshield. ORCID: 0009-0006-0211-4812.

**BibTeX:**

```bibtex
@article{owens2026synapshield,
  author = {Owens, Steven},
  title = {SynapShield: Intercepting Parkinson's Disease at the Gut-Brain Interface},
  journal = {medRxiv},
  year = {2026},
  month = {jun},
  doi = {10.1101/TBD},
  url = {https://github.com/artistso/synapshield},
  orcid = {0009-0006-0211-4812},
  note = {Preprint under review. Dedicated to Richard.}
}

@software{synapshield2026,
  author = {Owens, Steven},
  title = {SynapShield: Parkinson's Interception Technology},
  year = {2026},
  publisher = {GitHub},
  version = {v1.0.1},
  url = {https://github.com/artistso/synapshield},
  doi = {10.5281/zenodo.TBD},
  orcid = {0009-0006-0211-4812}
}
```

---

*"Hope, not just science." — From PDEs to Richard's tablet.*
